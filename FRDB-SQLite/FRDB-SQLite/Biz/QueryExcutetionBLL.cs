...
namespace FRDB_SQLite
{
    public class QueryExcutetionBLL
    {
        ...
        public QueryExcutetionBLL(String queryText, FdbEntity fdbEntity)
        {
            ...
            condition = new QueryConditionBLL(fdbEntity);
            ...
            try
            {
                if (this._selectedRelationTexts != null && this._selectedRelationTexts.Count() == 2)
                {
                    ...
                    FzRelationEntity secondRelation = this._relationSet
                        .Find(item => this._selectedRelationTexts[1].Equals(item.RelationName, StringComparison.InvariantCultureIgnoreCase));
                    FzSchemeEntity firstScheme = this._fdbEntity.Schemes
                        .Find(item => this._selectedRelationTexts[0].Substring(1).Insert(0, "s").Equals(item.SchemeName, StringComparison.InvariantCultureIgnoreCase));
                    FzSchemeEntity secondScheme = this._fdbEntity.Schemes
                        .Find(item => this._selectedRelationTexts[1].Substring(1).Insert(0, "s").Equals(item.SchemeName, StringComparison.InvariantCultureIgnoreCase));
                    List<FzAttributeEntity> joinAttrs = new List<FzAttributeEntity>();
                    foreach (FzAttributeEntity attr in firstScheme.Attributes)
                    {
                        if (attr.AttributeName != "µ")
                        {
                    ...
                    FdbEntity newFdbJoin = new FdbEntity()
                    {
                        ...
                        Schemes = joinSchemes,
                        DiscreteFuzzyNumbers = this._fdbEntity.DiscreteFuzzyNumbers,
                        ContinuousFuzzyNumbers = this._fdbEntity.ContinuousFuzzyNumbers
                    };
                    ...
                    this._selectedRelations = joinRelations;
                }
        ...
        private string getFZValue(string firstUvalue, string secondUValue, FdbEntity _fdbEntity, QueryConditionBLL queryBLL)
        {
            ...
            FzContinuousFuzzySetEntity get_fconFS = ContinuousFuzzySetBLL.GetConFNByName(firstUvalue, _fdbEntity);
            FzContinuousFuzzySetEntity get_sconFS = ContinuousFuzzySetBLL.GetConFNByName(secondUValue, _fdbEntity);
            ConFS conFS_fuRelation = null;
            ConFS conFS_suRelation = null;
            DisFS disFS_fuRelation = null;
            DisFS disFS_suRelation = null;
            if (get_fconFS != null)
            {
                conFS_fuRelation = new ConFS(get_fconFS.Name, get_fconFS.Bottom_Left, get_fconFS.Top_Left, get_fconFS.Top_Right, get_fconFS.Bottom_Right);
                disFS_fuRelation = QueryConditionBLL.transContoDis(conFS_fuRelation); //trans CF to DF
            }
            else
            {
                FzDiscreteFuzzySetEntity get_fdisFS = DiscreteFuzzySetBLL.GetDisFNByName(firstUvalue, _fdbEntity);
                if (get_fdisFS != null)
                    disFS_fuRelation = new DisFS(get_fdisFS.Name, get_fdisFS.V, get_fdisFS.M, get_fdisFS.ValueSet, get_fdisFS.MembershipSet);
                else if (!IsNumber(firstUvalue))
                    return "FN not exists";
            }
            if (get_sconFS != null)
            {
                conFS_suRelation = new ConFS(get_sconFS.Name, get_sconFS.Bottom_Left, get_sconFS.Top_Left, get_sconFS.Top_Right, get_sconFS.Bottom_Right);
                disFS_suRelation = QueryConditionBLL.transContoDis(conFS_suRelation); //trans CF to DF
            }
            else
            {
                FzDiscreteFuzzySetEntity get_sdisFS = DiscreteFuzzySetBLL.GetDisFNByName(secondUValue, _fdbEntity);
                if (get_sdisFS != null)
                    disFS_suRelation = new DisFS(get_sdisFS.Name, get_sdisFS.V, get_sdisFS.M, get_sdisFS.ValueSet, get_sdisFS.MembershipSet);
                else if (!IsNumber(secondUValue))
                    return "FN not exists";
            }
            if (conFS_fuRelation != null || disFS_fuRelation != null)
                check_ufRelation = true; //_uRelation is a fuzzyset
            if (conFS_suRelation != null || disFS_suRelation != null)
                check_usRelation = true; //_uRelation is a fuzzyset
            if (!check_ufRelation && !check_usRelation)
            {
                return Math.Min(Double.Parse(firstUvalue), Double.Parse(secondUValue)).ToString();
            }
            if (!check_ufRelation)
            {
                disFS_fuRelation = new DisFS();
                disFS_fuRelation.ValueSet.Add(Double.Parse(firstUvalue));
                disFS_fuRelation.MembershipSet.Add(1);
            }
            if (!check_usRelation)
            {
                disFS_suRelation = new DisFS();
        ...
        private void GetSelectedRelation()
        {
            try
            {
                foreach (String relationText in this._selectedRelationTexts)
                {
                    foreach (FzRelationEntity item in RelationSet)
                    {
                        if (item.RelationName.ToLower().Equals(relationText))
                        {
        ...
        {
            condition.FdbEntity = _fdbEntity;
                ...
                if(r.ValuesOnPerRow.Count > 0)
                {
                        ...
                        else if (itemSelects.Count > 0)
                        {
                            ...
                            foreach (Item item in itemSelects)
                            {
                                ...
                                foreach (var itemTuple in resultTuple)
                                {
                                    ...
                                    for (int j = 0; j < itemTuple.ValuesOnPerRow.Count; j++)
                                    {
                                        if (int.Parse(item.elements[0]) == j && item.aggregateFunction != "min" && item.aggregateFunction != "max")
                                        {
                                            ...
                                            FSName1 = condition.FindAndMarkFuzzy(itemTuple.ValuesOnPerRow[itemTuple.ValuesOnPerRow.Count - 1].ToString(), FSName1);
                                            break;
                                        }
                                        else if (int.Parse(item.elements[0]) == j && (item.aggregateFunction == "min" || item.aggregateFunction == "max"))
                                        {
                                            ...
                                            if(k == resultTuple.Count  && countMinMax > 1)
                                            {
                                                ...
                                                { 
                                                    ...
                                                    {
                                                        FSName2 = condition.FindAndMarkFuzzy(resultTuple[y].ValuesOnPerRow[itemTuple.ValuesOnPerRow.Count - 1].ToString(), FSName2);
                                                    }
                                                }
                                            }
                                ...
                                if (FSName1 != "")
                                    FSName = condition.FindAndMarkFuzzy(FSName1, FSName);
                                if (FSName2 != "")
                                    FSName = condition.FindAndMarkFuzzy(FSName2, FSName);
        ...
        {//the relations which user input such: select attr1, att2... from
            ...
            if (this._queryText.Contains("join"))
            {
                ...
                // select * from rPatient join rDiagnose
            ...
            for (int i = 0; i < condition.Length - 5; i++)
            {
                ...
                else
                {
                    if (i < (condition.Length - 8) && condition.Substring(i, 8) == "between ")
                    {
                        ...
                        if (isNotEle)
                        {
                            condition = condition.Remove(j, 4);
                            condition = condition.Insert(j, " or");
                        }
                    }
                    if (logic == " and ")
        ...
        private string checkDistinctSelect()
        {
            ...
            {
                if (i != 0 && this._selectedAttributeTexts[i].Contains("distinct"))
        ...
        private FzRelationEntity ProcessGroupBy(FzRelationEntity result)
        {
            ...
            //Như add List<List<Item>> listConditionForTuples = new List<List<Item>>();//having: condition1-true and condition2 false, to get priority tuple after process having condition.
            ...
            if (having > 0)
            {
                ...
                List<Item> itemConditionHavings = FormatCondition(tmp);
                bool checkAttr = false;
                //Check fuzzy set and object here
                this._errorMessage = ExistsFuzzySet(itemConditionHavings);
                for (int k = 0; k < itemConditionHavings.Count(); k++)
                {
                    for(int l = 0; l < indexGroupby.Count(); l++)
                    {
                        if ((itemConditionHavings[k].aggregateFunction == "" && itemConditionHavings[k].elements[0].ToString() == indexGroupby[l].ToString()) || itemConditionHavings[k].aggregateFunction != "")
                        {
                            checkAttr = true;
                            break;
                        }
                ...
                for (int j = 0; j < filterResult.Tuples.Count; j++)//format each tuple after group by
                {
                    ...
                    else if (filterResultHaving.Tuples.Count == 0) return filterResult;// when error occur, return safe result :)
